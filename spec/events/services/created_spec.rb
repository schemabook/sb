...
RSpec.describe Events::Services::Created do
  let(:user)    { create(:user) }
  let(:service) { create(:service, :with_team, name: "foo", created_by: user.id) }

  subject { described_class.new(record: service) }
  ...
  describe "#schema" do
    ...
    it "requires a record" do
      expect(JSON.parse(subject.schema)["name"]).to eq("service")
    end
  ...
  describe "#payload" do
    ...
    it "includes the service id" do
      expect(subject.payload[:after][:id]).to eq(service.id)
    end

    it "includes the service created_at timestamp" do
      expect(subject.payload[:source][:ts_ms]).to eq((service.created_at.to_f * 1000).to_i)
    end

    it "includes the service table name" do
      expect(subject.payload[:source][:table]).to eq(service.class.table_name)
    end

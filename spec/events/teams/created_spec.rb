...
RSpec.describe Events::Teams::Created do
  let(:user) { create(:user) }
  let(:team) { user.team }

  subject { described_class.new(record: team) }
  ...
  describe "#schema" do
    ...
    it "requires a record" do
      expect(JSON.parse(subject.schema)["name"]).to eq("team")
    end
  ...
  describe "#payload" do
    ...
    it "includes the team id" do
      expect(subject.payload[:after][:id]).to eq(team.id)
    end

    it "includes the team created_at timestamp" do
      expect(subject.payload[:source][:ts_ms]).to eq((team.created_at.to_f * 1000).to_i)
    end

    it "includes the team table name" do
      expect(subject.payload[:source][:table]).to eq(team.class.table_name)
    end

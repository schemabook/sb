...
RSpec.describe Events::Versions::Created do
  ...
  describe "#schema" do
    ...
    it "requires a record" do
      expect(JSON.parse(subject.schema)["name"]).to eq("version")
    end
  ...
  describe "#payload" do
    ...
    it "includes the version id" do
      expect(subject.payload[:after][:id]).to eq(version.id)
    end

    it "includes the schema id" do
      expect(subject.payload[:after][:schema]).to eq(version.schema.id)
    end

    it "includes the schema created_at timestamp" do
      expect(subject.payload[:source][:ts_ms]).to eq((version.created_at.to_f * 1000).to_i)
    end

    it "includes the schema table name" do
      expect(subject.payload[:source][:table]).to eq(version.class.table_name)
    end

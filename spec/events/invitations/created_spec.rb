...
RSpec.describe Events::Invitations::Created do
  ...
  describe "#schema" do
    ...
    it "requires a record" do
      expect(JSON.parse(subject.schema)["name"]).to eq("teammate")
    end
  ...
  describe "#payload" do
    ...
    it "includes the teammate id" do
      expect(subject.payload[:after][:id]).to eq(teammate.id)
    end

    it "includes the teammate created_at timestamp" do
      expect(subject.payload[:source][:ts_ms]).to eq((teammate.created_at.to_f * 1000).to_i)
    end

    it "includes the teammate table name" do
      expect(subject.payload[:source][:table]).to eq(teammate.class.table_name)
    end

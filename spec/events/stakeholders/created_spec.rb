...
RSpec.describe Events::Stakeholders::Created do
  ...
  let(:stakeholder) { create(:stakeholder, user: user, schema: schema) }
  ...
  describe "#schema" do
    ...
    it "requires a record" do
      expect(JSON.parse(subject.schema)["name"]).to eq("stakeholder")
    end
  ...
  describe "#payload" do
    ...
    it "includes the team id" do
      expect(subject.payload[:after][:id]).to eq(stakeholder.id)
    end

    it "includes the team created_at timestamp" do
      expect(subject.payload[:source][:ts_ms]).to eq((stakeholder.created_at.to_f * 1000).to_i)
    end

    it "includes the team table name" do
      expect(subject.payload[:source][:table]).to eq(stakeholder.class.table_name)
    end

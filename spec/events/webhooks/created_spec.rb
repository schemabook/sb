...
RSpec.describe Events::Webhooks::Created do
  let(:user)     { create(:user) }
  let(:format)   { create(:format, file_type: :json) }
  let(:schema)   { create(:schema, team: user.team, name: "foo", format:) }
  let(:webhook)  { create(:webhook, schema:, user:, url: "https://example.com") }

  subject { described_class.new(record: webhook) }
  ...
  describe "#schema" do
    ...
    it "requires a record" do
      expect(JSON.parse(subject.schema)["name"]).to eq("webhook")
    end
    ...
    context "after block" do
      ...
      it "required attributes" do
        ...
        # schema
        expect(block["type"]["fields"][1]["name"]).to eq("schema")
        expect(block["type"]["fields"][1]["type"]).to eq("int")
        # url
        expect(block["type"]["fields"][2]["name"]).to eq("url")
        expect(block["type"]["fields"][2]["type"]).to eq("string")
      end
    end
  ...
  describe "#payload" do
    ...
    it "includes the webhook id" do
      expect(subject.payload[:after][:id]).to eq(webhook.id)
    end

    it "includes the schema id" do
      expect(subject.payload[:after][:schema]).to eq(webhook.schema.id)
    end

    it "includes the schema created_at timestamp" do
      expect(subject.payload[:source][:ts_ms]).to eq((webhook.created_at.to_f * 1000).to_i)
    end

    it "includes the schema table name" do
      expect(subject.payload[:source][:table]).to eq(webhook.class.table_name)
    end

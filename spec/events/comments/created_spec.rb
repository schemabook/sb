...
RSpec.describe Events::Comments::Created do
  ...
  describe "#schema" do
    ...
    it "requires a record" do
      expect(JSON.parse(subject.schema)["name"]).to eq("comment")
    end
  ...
  describe "#payload" do
    ...
    it "includes the comment id" do
      expect(subject.payload[:after][:id]).to eq(comment.id)
    end

    it "includes the comment created_at timestamp" do
      expect(subject.payload[:source][:ts_ms]).to eq((comment.created_at.to_f * 1000).to_i)
    end

    it "includes the comment table name" do
      expect(subject.payload[:source][:table]).to eq(comment.class.table_name)
    end

class Activity < ApplicationRecord
  ...
  scope :for_schema, ->(schema) { where(resource_class: 'Schema', resource_id: schema.id).or(where(resource_class: 'Version', resource_id: schema.versions.pluck(:id))).or(where(resource_class: 'Webhook', resource_id: schema.webhooks.pluck(:id))) }
